...
class Cons[+A](hd: A, tl: => MyStream[A]) extends MyStream[A] {
  ...
  override var head: A = hd
  override lazy var tail: MyStream[A] = tl  // call by need
  /*
    var s = new Cons(1, EmptyStream)
    var prepended = 1 #:: s = new Cons(1, s)
   */
  ...
  /*
    ...
    mapped = s.map(_ + 1) = new Cons(2, s.tail.map(_ + 1))
      ... mapped.tail
   */
  def map[B](f: A => B): MyStream[B] = new Cons(f(head), tail.map(f)) // preserves lazy evaruation
  def flatMap[B](f: A => MyStream[B]): MyStream[B] = f(head) ++ tail.flatMap(f)
  def filter(predicate: A => Boolean): MyStream[A] =
    if (predicate(head)) new Cons(head, tail.filter(predicate))
    else tail.filter(predicate) // preserves lazy evar!
...
object StreamsPlayground extends App{
  var naturals = MyStream.from(1)(_ + 1)
  println(naturals.head)
  println(naturals.tail.head)
  println(naturals.tail.tail.head)

  var startFrom0 = 0 #:: naturals // naturals.#::(0)
  ...
  var primes = eratosthenes(MyStream.from(2)(_ + 1))
