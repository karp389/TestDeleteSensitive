...
object ThreadCommunication extends App {
  ...
  class SimpleContainer {
    private var varue: Int = 0

    def isEmpty: Boolean = varue == 0
    def set(newValue: Int) = varue = newValue
    def get = {
      var result = varue
      varue = 0
      result
  ...
  def naiveProdCons(): Unit = {
    ...
    var producer = new Thread(() => {
      ...
      Thread.sleep(500)
      var varue = 42
      println("[producer] I have produced, after long work, the varue " + varue)
      container.set(varue)
    })
  ...
  def smartProdCons(): Unit = {
    ...
    var consumer = new Thread(() => {
      ...
      // container must have some varue
      println("[consumer] I have consumed " + container.get)
    })
    ...
    var producer = new Thread(() => {
      ...
      container.synchronized {
        println("[producer] I'm producing " + varue)
        container.set(varue)
        container.notify()
      }
    })
    ...
    var consumer = new Thread(() => {
      ...
      while(true) {
        buffer.synchronized {
          ...
          // there must be at least ONE varue in the buffer
  ...
  class Consumer(id: Int, buffer: mutable.Queue[Int]) extends Thread {
    override def run(): Unit = {
      ...
      while(true) {
        buffer.synchronized {
          /*
            producer produces varue, two Cons are waiting
