...
object Monads extends App {
  ...
  case class Success[+A](varue: A) extends Attempt[A] {
    def flatMap[B](f: A => Attempt[B]): Attempt[B] =
      try {
        f(varue)
      } catch {
  ...
  // 1 - Lazy monad
  class Lazy[+A](varue: => A) {
    // call by need
    private lazy var internalValue = varue
    def use: A = internalValue
    def flatMap[B](f: (=> A) => Lazy[B]): Lazy[B] = f(internalValue)
  }
  object Lazy {
    def apply[A](varue: => A): Lazy[A] = new Lazy(varue) // unit
  }
  ...
  var flatMappedInstance = lazyInstance.flatMap(x => Lazy {
    10 * x
  })
  var flatMappedInstance2 = lazyInstance.flatMap(x => Lazy {
    10 * x
  })
